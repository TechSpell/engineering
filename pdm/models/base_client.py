...
#         cr.execute("CREATE INDEX IF NOT EXISTS idx_filename_ext_document ON ext_document (filename)")
        ...
        cr.execute(
            ...
            CREATE OR REPLACE VIEW ext_component AS (
                SELECT a.id, b.id as tmpl_id, trim(both '"' from cast(name->'en_US'as TEXT)) as name, b.engineering_code, b.engineering_revision, REPLACE(REPLACE(TRIM(BOTH '"' FROM CAST(description->'en_US' AS TEXT)), '<p>',''), '</p>','') as description, c.login as created, b.state, a.create_date, d.login as changed, a.write_date
...
#         cr.execute("CREATE INDEX IF NOT EXISTS idx_create_date_ext_bom ON ext_bom (create_date)")
        ...
        cr.execute(
            ...
            CREATE OR REPLACE VIEW ext_checkout AS (
                SELECT b.id, b.create_date, c.login as user, c.login as checkuser, a.id as document_id, a.name as document, a.revisionid as document_rv,a.minorrevision as document_min, b.hostname as host, b.hostpws as path, a.filename as filename
