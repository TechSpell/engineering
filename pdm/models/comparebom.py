...
class plm_compare_bom(orm.TransientModel):
    ...
    _columns = {
        ...
        'type_id1': fields.selection(
            [('normal', 'Normal BoM'), ('phantom', 'Sets / Phantom'), ('ebom', 'Engineering BoM'),
             ('spbom', 'Spare BoM')], 'BoM Type'),
        'part_id1': fields.many2one('product.product', 'Part', ondelete='cascade'),
        'revision1': fields.related('part_id1', 'engineering_revision', type="integer", relation="product.template",
                                    string="Revision", store=False),
        'description1': fields.related('part_id1', 'description', type="char", relation="product.template",
                                       string="Description", store=False),
        'bom_id2': fields.many2one('mrp.bom', 'BoM 2', required=True, ondelete='cascade'),
        'type_id2': fields.selection(
            [('normal', 'Normal BoM'), ('phantom', 'Sets / Phantom'), ('ebom', 'Engineering BoM'),
             ('spbom', 'Spare BoM')], 'BoM Type'),
        'part_id2': fields.many2one('product.product', 'Part', ondelete='cascade'),
        'revision2': fields.related('part_id2', 'engineering_revision', type="integer", relation="product.template",
                                    string="Revision", store=False),
        'description2': fields.related('part_id2', 'description', type="char", relation="product.template",
                                       string="Description", store=False),
    ...
    def action_compare_Bom(self, cr, uid, ids, context=None):
        """
        ...
        checkObj = self.browse(cr, uid, ids[0], context=context)
        ...
        defaults = {'name': checkObj.bom_id1.product_id.name, 'type_id1': checkObj.bom_id1.type,
                    'part_id1': checkObj.bom_id1.product_id.id, 'type_id2': checkObj.bom_id2.type,
                    'part_id2': checkObj.bom_id2.product_id.id}
        self.write(cr, uid, ids, defaults, context=context)
    ...
    def _compare_Bom(self, cr, uid, oid1=False, oid2=False, context=None):
        """
    ...
    def _differs_Bom(self, cr, uid, oid1=False, oid2=False, fields=[], context=None):
        """
            Create a new Normal Bom (recursive on all EBom children)
        """
        defaults = {}
...
class plm_missing_bom(orm.TransientModel):
    _name = "plm.missing.bom"
    _description = "BoM Missing Objects"
    _columns = {
        'bom_id': fields.many2one('plm.compare.bom', 'BoM', ondelete='cascade'),
        'bom_idrow': fields.many2one('mrp.bom', 'BoM Line', ondelete='cascade'),
        'part_id': fields.many2one('product.product', 'Part', ondelete='cascade'),
        'revision': fields.related('part_id', 'engineering_revision', type="integer", relation="product.template",
                                   string="Revision", store=False),
        'description': fields.related('part_id', 'description', type="char", relation="product.template",
                                      string="Description", store=False),
        'itemnum': fields.related('bom_idrow', 'itemnum', type="integer", relation="mrp.bom",
                                  string="CAD Item Position", store=False),
        'itemqty': fields.related('bom_idrow', 'product_qty', type="float", relation="mrp.bom", string="Quantity",
                                  store=False),
        'reason': fields.char(string="Difference", size=32)
    }


class plm_adding_bom(orm.TransientModel):
