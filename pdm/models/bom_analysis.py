...
class plm_check_bom(models.TransientModel):
    _name = "plm.check.bom"
    _description = "Products to be checked"
    
    temp_id         =   fields.Many2one ('plm.temporary', index=True,               string='Replace Check',  ondelete='cascade')
    bom_id          =   fields.Many2one ('mep.bom', index=True,                     string='Bom',            ondelete='cascade')
    part_id         =   fields.Many2one ('product.template', index=True,            string='Father',         ondelete='cascade')
    name            =   fields.Char     (related="part_id.name",                    string="BoM Product",    store=False)
    ...
    child_id        =   fields.Many2one ('product.product', index=True,             string='Child',          ondelete='cascade')
    ch_name         =   fields.Char     (related="child_id.name",                   string="ChildProduct",   store=False)
    ch_revision     =   fields.Integer  (related="child_id.engineering_revision",   string="Revision",       store=False)
    ch_status       =   fields.Selection(related="child_id.state",                  string="Status",         store=False)
    ch_description  =   fields.Html     (related="child_id.description",            string="Description",    store=False)
    ch_reason       =   fields.Char     (string="Notes",                                                                )
...
class plm_temporary(models.TransientModel):
    ...
    ch_bom_ids      =   fields.One2many ('plm.check.bom',  'temp_id',     index=True, string='Boms in which to replace products'   )
